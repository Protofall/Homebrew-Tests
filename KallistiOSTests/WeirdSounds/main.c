...
int main(int argc, char **argv){
	...
	audio_info_t infoFX1, infoFX2, infoFX3;
	audio_source_t sourceFX1, sourceFX2, sourceFX3;

	//Setup
	#ifdef _arch_dreamcast
		if(audio_load_WAV_file_info("/rd/cursor_back.wav", &infoFX1, AUDIO_NOT_STREAMING) == AL_FALSE){return 1;}
	#else
		if(audio_load_WAV_file_info("romdisk/cursor_back.wav", &infoFX1, AUDIO_NOT_STREAMING) == AL_FALSE){return 1;}
	#endif
	
	if(audio_create_source(&sourceFX1, &infoFX1, (vec2_f_t){0,0}, AL_TRUE, 0.5, 1) == AL_FALSE){return 1;}

	//Setup
	#ifdef _arch_dreamcast
		if(audio_load_WAV_file_info("/rd/cursor_move.wav", &infoFX2, AUDIO_NOT_STREAMING) == AL_FALSE){return 1;}
	#else
		if(audio_load_WAV_file_info("romdisk/cursor_move.wav", &infoFX2, AUDIO_NOT_STREAMING) == AL_FALSE){return 1;}
	#endif
	
	if(audio_create_source(&sourceFX2, &infoFX2, (vec2_f_t){0,0}, AL_TRUE, 0.5, 1) == AL_FALSE){return 1;}

	//Setup
	#ifdef _arch_dreamcast
		if(audio_load_WAV_file_info("/rd/cursor_select.wav", &infoFX3, AUDIO_NOT_STREAMING) == AL_FALSE){return 1;}
	#else
		if(audio_load_WAV_file_info("romdisk/cursor_select.wav", &infoFX3, AUDIO_NOT_STREAMING) == AL_FALSE){return 1;}
	#endif
	
	if(audio_create_source(&sourceFX3, &infoFX3, (vec2_f_t){0,0}, AL_TRUE, 0.5, 1) == AL_FALSE){return 1;}
	...
	audio_source_t * target_source = &sourceFX1;
	...
	audio_free_source(&sourceFX3);

	audio_free_info(&infoFX1);
	audio_free_info(&infoFX2);
	audio_free_info(&infoFX3);
